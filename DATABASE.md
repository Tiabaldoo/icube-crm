...
`directions`, `sites`, `teachers`, `teacher_projects`, `teacher_project_directions`, `projects`, `study_groups` — справочники и регулярное расписание. Отсутствие пары `(teacher_id, project_id)` означает, что преподаватель не работает в проекте; существующая пара хранит активное или неактивное состояние, а разрешённые направления — `teacher_project_directions`. `teacher_directions` сохраняется как историческая совместимость до отдельной безопасной миграции удаления, но доступность новых назначений определяется активной проектной связью и направлением.
...
Миграция `020_payment_receipts.sql` добавляет приватные metadata чеков в `payment_receipts` и явную many-to-many связь `payment_receipt_payments`. Сам чек не влияет на баланс. Статус для родителя определяется наличием хотя бы одной действующей связанной payment; закрытие очереди сотрудником хранится отдельно и без payment не подтверждает оплату.
