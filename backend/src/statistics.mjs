...
export function calculateStatistics({ period, filters, attendanceRows = [], firstVisitRows = [], leftRows = [], currentRows = [] }) {
  ...
  for (const row of attendanceRows) {
    ...
    const groupId = String(row.group_id); const item = historicalGroups.get(groupId) ?? { lessonIds: new Set(), visits: 0, absences: 0, newChildren: 0, leftEnrollments: 0 };
    item.lessonIds.add(lessonId);
    if (row.attendance_id != null && !row.is_trial && row.marked_at != null) {
  ...
  const groups = [...currentGroups.values()].map((group) => {
    ...
    return { id: group.id, name: group.name, projectId: group.projectId, projectName: group.projectName,
      directionId: group.directionId, directionName: group.directionName, currentMembers: group.children.size, capacity: CAPACITY,
      occupancyPercent: percent(group.children.size, CAPACITY), completedLessons: history.lessonIds.size,
  ...
  const averageOccupancy = groups.length ? groups.reduce((sum, group) => sum + group.currentMembers * 100 / CAPACITY, 0) / groups.length : 0;
...
export function createStatistics(pool) {
  async function get(raw = {}) {
    ...
    const [[attendanceRows], [firstVisitRows], [leftRows], [currentRows]] = await Promise.all([
      pool.query(`SELECT l.id lesson_id,l.group_id,a.id attendance_id,a.attendance_type,a.present,a.is_trial,a.marked_at
        FROM lessons l LEFT JOIN attendances a ON a.lesson_id=l.id
