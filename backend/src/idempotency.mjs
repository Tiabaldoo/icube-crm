...
export function requireIdempotencyKey(value) {
  ...
  if (!key || key.length > 128) {
    throw new ApiProblem(400, 'IDEMPOTENCY_KEY_REQUIRED', 'Для операции нужен Idempotency-Key');
  }
  return key;
}
...
export function scopedIdempotencyKey({ key, actorUserId, operation, projectId, entity }) {
  ...
  if (!actor) throw new ApiProblem(401, 'UNAUTHENTICATED', 'Для операции требуется пользователь');
