...
export function createMysqlCatalog(pool, { siteRent = createSiteRentService(pool), notificationEvents = null } = {}) {
    ...
    try {
      const result = await inTransaction(pool, async (connection) => {
        ...
        if (creating) {
          ...
          if (enrollmentInput.groupId != null) {
            const groupId = id(enrollmentInput.groupId, 'groupId');
            const [membershipResult] = await connection.query(`INSERT INTO group_memberships
              ...
              enrollmentId: resolvedEnrollmentId, groupId, startedOn: businessDate(),
            });
