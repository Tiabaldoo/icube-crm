...
Каждая изменяемая сущность имеет `updated_at`, а занятие — `lock_version`. Транзакционная блокировка строки занятия делает повторные start/finish идемпотентными. Повторное завершение возвращает уже полученный результат и не создаёт второе списание; для каждого attendance сервис отдельно проверяет отсутствие активного, ещё не обращённого ledger effect.

Auth middleware получает actor только из проверенной server-side session; команды занятий дополнительно защищены проверкой teacher scope, состояния под блокировкой строки и уникальными связями ledger/reversal. Start, attendance, extras и finish безопасны при повторе по состоянию серверных строк; quick child использует scoped `Idempotency-Key` и существующий уникальный ключ карточки ребёнка.

Действия преподавателя в уже открытом занятии сначала записываются в IndexedDB и сразу применяются к локальному UI. Очередь синхронизирует start → состав/посещаемость → finish последовательно, объединяет повторные изменения attendance до последнего состояния и хранит соответствие временного ID нового ребёнка серверному ID. Очередь фотографий остаётся отдельной, но ждёт зависимые команды занятия и получает заменённый ID ребёнка перед загрузкой.
