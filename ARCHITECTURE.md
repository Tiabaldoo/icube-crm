...
До включения настоящей авторизации команды занятий защищены проверкой состояния под блокировкой строки и уникальными связями ledger/reversal. Полный replay результата по `Idempotency-Key` через таблицу `idempotency_keys` подключается вместе с проверенным `user_id`.
