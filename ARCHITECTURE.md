...
Auth middleware получает actor только из проверенной server-side session; команды занятий дополнительно защищены проверкой teacher scope, состояния под блокировкой строки и уникальными связями ledger/reversal. Полный replay результата по `Idempotency-Key` остаётся отдельной задачей, но actor `user_id` уже проверен.
