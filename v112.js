...
(function () {
  ...
  window.lesson=function(){
    ...
    return '<button class="btn" style="margin-bottom:14px" onclick="navTo(\'calendar\')">← Календарь</button>'+
      ...
      '<div class="info-line"><span>Группа</span><b>'+g.name+'</b></div><div class="info-line"><span>Фактический преподаватель</span><b>'+t.name+'</b></div><div class="info-line"><span>Тема</span><b>'+(l.topic||'Не указана')+'</b></div><div class="info-line"><span>Присутствовало</span><b>'+Object.values(l.attendance||{}).filter(Boolean).length+(l.extras||[]).length+'</b></div>'+
      ...
      kids.map(function(c){return '<div class="kpi-line"><b>'+c.name+'</b><span class="badge '+(l.attendance[c.id]?'green':'gray')+'">'+(l.attendance[c.id]?'Был':'Не отмечен')+'</span></div>';}).join('')+'</div></div>';
  };
