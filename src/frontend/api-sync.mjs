...
function installAuthenticatedShells() {
  const pushBell = () => `<button class="push-bell-button" type="button" data-push-bell onclick="const popover=()=>document.querySelector('.push-popover')?'yes':'no';const push=window.icubePush;const hasToggle=!!(push&&typeof push.togglePanel==='function');window.alert('bell tap\\nicubePush: '+(push?'yes':'no')+'\\ntogglePanel: '+(hasToggle?'yes':'no')+'\\npopover before: '+popover());if(hasToggle){let error='';try{push.togglePanel(this)}catch(err){error=err?.message||String(err)}if(error){window.alert('togglePanel error: '+error)}else{requestAnimationFrame(()=>{const p=document.querySelector('.push-popover');if(!p){window.alert('popover diagnostics\\npopover after: no');return}const r=p.getBoundingClientRect();const s=getComputedStyle(p);const vv=window.visualViewport;const x=r.left+r.width/2;const y=r.top+Math.min(r.height/2,20);const e=document.elementFromPoint(x,y);const name=e?e.tagName+(e.className?'.'+String(e.className).trim().replace(/\\s+/g,'.'):''):'null';window.alert('popover diagnostics\\nrect.top: '+r.top+'\\nrect.left: '+r.left+'\\nrect.right: '+r.right+'\\nrect.bottom: '+r.bottom+'\\nrect.width: '+r.width+'\\nrect.height: '+r.height+'\\nviewport: '+window.innerWidth+'x'+window.innerHeight+'\\nvisualViewport: '+(vv?vv.width+'x'+vv.height:'n/a')+'\\ndisplay: '+s.display+'\\nvisibility: '+s.visibility+'\\nopacity: '+s.opacity+'\\nposition: '+s.position+'\\nz-index: '+s.zIndex+'\\nbackground: '+s.backgroundColor+'\\ntransform: '+s.transform+'\\nelementFromPoint: '+name)})}}" aria-label="Системные уведомления" title="Системные уведомления"><span data-push-bell-icon>${window.icubePush?.icon?.() ?? '🔕'}</span></button>`;
