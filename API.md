...
Первый финансовый срез реализует `GET/POST/PATCH/DELETE /payments` и `GET /balances`. Создание, исправление и удаление оплаты меняют её единственную запись в `balance_entries` и баланс соответствующего enrollment в одной транзакции. Оплату нельзя удалить, пока её lot имеет активное потребление; полностью reversed потребление очищается вместе с lot. Возвраты пока не подключены.

Срез занятий реализует материализацию occurrence из расписания групп, `GET/POST/PATCH /lessons`, start/cancel/empty-trip/finish, attendance, extras, quick child и чтение `salary-accruals`. Start фиксирует основной roster в `lesson_roster_members`; последующие изменения группы его не меняют. Обычное присутствие списывает один урок по FIFO из `balance_lots`, а нехватка оплаченных lots оставляет разрешённый отрицательный баланс. Исправление проведённого attendance создаёт reversal исходного ledger effect, восстанавливает те же lots и создаёт новый эффект при необходимости. Директорское удаление ошибочного attendance после reversal физически очищает его технический ledger; явно отмеченное отсутствие остаётся историей. Зарплата начисляется фактическому преподавателю по versioned rate; прежнее начисление помечается `reversed_at`, новое ссылается на него через `supersedes_accrual_id`.

Маршруты возвратов, переводов баланса, фотографий, партнёрских расчётов и статистики пока возвращают `501 NOT_IMPLEMENTED`.
