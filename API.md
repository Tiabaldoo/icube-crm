...
Финансовый срез реализует `GET/POST/PATCH/DELETE /payments`, `GET /balances` и `GET/POST/DELETE /refunds`. Возврат всегда связан с исходной оплатой, уменьшает её свободный lot по исторической цене и меняет баланс enrollment в одной транзакции. Оплату нельзя изменить или удалить при активном возврате; после удаления всех возвратов действуют обычные ограничения по активным consumptions.
...
Маршруты переводов баланса, фотографий, партнёрских расчётов и статистики пока возвращают `501 NOT_IMPLEMENTED`.
